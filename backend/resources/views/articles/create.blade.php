    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div class="bg-white shadow rounded-lg overflow-hidden">
            <div class="bg-gray-100 px-6 py-4 border-b">
                <h5 class="text-lg font-semibold text-gray-700">Create New Article</h5>
            </div>
            <form action="{{ route('articles.store') }}" method="POST" class="px-6 py-6">
                @csrf
                ...
                <div class="mb-4">
                    ...
                    <textarea name="body" id="body" rows="10" required
                        class="w-full rounded border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 @error('body') border-red-500 @enderror">{{ old('body') }}</textarea>
                    @error('body')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex items-center justify-between mt-6">
                    ...
                    <button type="submit" class="bg-indigo-500 hover:bg-indigo-600 text-white py-2 px-4 rounded transition duration-200">Create Article</button>
                </div>
            </form>
        </div>
    </div>
